...
try:
    ...
    if df is not None:
        ...
        with st.spinner('📱 正在同步全球行情與配息...'):
            ...
            for index, row in df.iterrows():
                ...
                try:
                    ...
                    div_map[sym] = float(divs[divs.index > one_year_ago].sum()) if not divs.empty else 0
                except:
                    div_map[sym] = 0
        ...
        calc_cols = ['current_price', 'mv_twd', 'profit_twd', 'roi', 'net_div_twd', 'drawdown_52h', 'daily_chg_twd']
        df[calc_cols] = df.apply(calculate_metrics, axis=1)

        # --- 4. 數據統計 (強化轉換，避免 KeyError) ---
        ...
        if history_list:
            st.markdown("---")
            history_combined = pd.concat(history_list, axis=1).fillna(method='ffill').fillna(0)
