...
try:
    ...
    # --- 6. 核心修正：計算 3 個月歷史 (確保端點絕對一致) ---
    with st.spinner('計算歷史趨勢對齊中...'):
        history_combined = pd.DataFrame()
        for sym in unique_symbols:
            # 抓取歷史時關閉 auto_adjust 以免股價被平滑
            h_data = yf.Ticker(sym).history(period="3mo", auto_adjust=False)['Close']
        ...
        # 【修正 700 萬落差的關鍵步】: 
        # 直接把趨勢圖最後一個日期點設定為「現在」，並強制設定為摘要區的數值
        now_label = pd.Timestamp.now(tz='Asia/Taipei').floor('min')
        trend_data.loc[now_label] = total_mv 
        trend_data = trend_data.sort_index().tail(90) # 只留最近 90 天
    ...
    # 修正 Y 軸範圍，避免 7000 萬的資產看起來像平的一條線
    fig_trend.update_layout(
        hovermode="x unified", template="plotly_white", height=450,
        yaxis=dict(tickformat=",.0f", range=[trend_data['Total_MV'].min()*0.9, trend_data['Total_MV'].max()*1.05])
    )
    ...
    with k2:
        ...
        # 計算跌幅邏輯維持不變
        risk_df['較高點跌幅 %'] = ((risk_df['current_price'] - df['h52']) / df['h52'] * 100)
