...
try:
    ...
    # --- 4. 數據同步 (歷史與即時，含 700 萬修正邏輯) ---
    with st.spinner('正在同步全球即時行情...'):
    ...
    def process_row(row):
        ...
        return pd.Series([curr_price, mv_twd, profit_twd, roi, net_div_twd, yield_rate, daily_change])
    ...
    trend_data.iloc[-1] = total_mv # 強制最後一點等於當前總值
