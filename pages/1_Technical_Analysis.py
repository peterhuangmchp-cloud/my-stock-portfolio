...
if 'gsheet_id' in st.session_state:
    try:
        # 讀取股票清單
        url = f"https://docs.google.com/spreadsheets/d/{st.session_state['gsheet_id']}/export?format=csv&gid=0"
        ...
        # 獲取歷史數據
        df = yf.download(selected_stock, period=period)
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['BBU_20_2.0'], line=dict(color='rgba(255,255,255,0.2)'), name="上軌"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df['BBL_20_2.0'], line=dict(color='rgba(255,255,255,0.2)'), name="下軌"), row=1, col=1)
        ...
        # MACD
        fig.add_trace(go.Bar(x=df.index, y=df['MACDh_12_26_9'], name="Histogram"), row=4, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df['MACD_12_26_9'], name="MACD"), row=4, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df['MACDs_12_26_9'], name="Signal"), row=4, col=1)
