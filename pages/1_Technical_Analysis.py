...
if 'gsheet_id' in st.session_state:
    try:
        url = f"https://docs.google.com/spreadsheets/d/{st.session_state['gsheet_id']}/gviz/tq?tqx=out:csv&gid=0"
        stock_list = pd.read_csv(url)['代號'].tolist()
        
        selected_stock = st.selectbox("選擇要分析的標的", stock_list)
        period = st.select_slider("選擇分析區間", options=["3mo", "6mo", "1y", "2y", "5y"], value="1y")
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['BBU_20_2.0'], line=dict(color='gray', width=1), name="布林上軌"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df['BBL_20_2.0'], line=dict(color='gray', width=1), name="布林下軌"), row=1, col=1)
        ...
        st.error(f"技術分析載入失敗: {e}")
else:
    st.info("請先回到主頁載入資產清單。")
