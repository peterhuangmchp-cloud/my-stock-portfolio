...
# 這裡很重要：請將 gid 換成您 Google Sheet 中 "History" 分頁的數字
# 您可以在瀏覽器打開 History 分頁，看網址最後面的 gid=xxxxxx
history_gid = "1878368812" # <-- 這是範例，請替換成您實際的 GID

def load_history_data(sheet_id, gid):
    url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=csv&gid={362865924}"
...
st.markdown(f"數據來源：Google Sheet - History 分頁")
...
try:
    ...
    if history_df is not None and not history_df.empty:
        ...
        with st.expander("查看所有歷史數據明細"):
            st.dataframe(history_df.sort_values('date', ascending=False).style.format({
                'total_mv': '{:,.0f}',
                'daily_perf': '{:,.0f}'
            }), use_container_width=True)

    else:
        st.warning("目前 History 分頁中還沒有數據。請確認 Google Apps Script 是否已成功執行第一次紀錄。")

except Exception as e:
    st.error(f"讀取失敗：{e}")
    st.info("請確認：1. Google Sheet 分頁名稱為 History。 2. 標題欄位包含 date, total_mv, daily_perf")
